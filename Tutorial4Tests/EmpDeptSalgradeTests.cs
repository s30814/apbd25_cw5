...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        var result = emps.Where(e => (from d in depts where d.Loc == "CHICAGO" select d.DeptNo).Contains(e.DeptNo)).ToList();
    ...
    public void ShouldCountEmployeesPerDepartment()
    {
        var emps = Database.GetEmps();
         ...
         var result= emps.GroupBy(e=>e.DeptNo).Select(d=> new {DeptNo=d.Key, Count=d.Count()}).ToList();
    ...
    public void ShouldReturnEmployeesWithCommission()
    {
        var emps = Database.GetEmps();
         ...
         var result = emps.Where(e => e.Comm.HasValue).Select(e => new {EName=e.EName, Comm=e.Comm}).ToList(); 
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var result = emps.SelectMany( e => grades.Where(s => e.Sal >= s.Losal && e.Sal <= s.Hisal),(e, s) => new {e.EName, s.Grade}).ToList();
    ...
    public void ShouldCalculateAverageSalaryPerDept()
    {
        ...
        var result = emps.GroupBy(e => e.DeptNo).Select(d => new {DeptNo = d.Key, AvgSal = d.Average(e=>e.Sal)}).ToList();
