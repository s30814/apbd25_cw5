...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldReturnEmployeeManagerPairs()
    {
        var emps = Database.GetEmps();
         ...
         var result = emps.SelectMany(e=> emps.Where(d=>d.EmpNo==e.Mgr), (e,d)=>new {Employee=e.EName, Manager=d.EName}).ToList();
    ...
    public void ShouldReturnTotalIncomeIncludingCommission()
    {
        var emps = Database.GetEmps();
         ...
         var result = emps.Select(e => new {e.EName, Total = e.Sal + (e.Comm.HasValue ? e.Comm.Value : 0)}).ToList();
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = (from e in emps join d in depts on e.DeptNo equals d.DeptNo from s in grades where e.Sal >= s.Losal && e.Sal <= s.Hisal select new{e.EName,d.DName,s.Grade}).ToList();
